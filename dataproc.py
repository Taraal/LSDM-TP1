...
INIT = Pig.compile("""
A = LOAD 'gs://scorfa_data_bucket/small_page_links.nt' using PigStorage(' ') as (url:chararray, p:chararray, link:chararray);
B = GROUP A by url;                                                                                  
...
params = { 'd': '0.5', 'docs_in': 'gs://scorfa_data_bucket/out/pagerank_data_simple' }
...
for i in range(3):
   out = "gs://scorfa_data_bucket/out/pagerank_data_" + str(i + 1)
   params["docs_out"] = out
   Pig.fs("rmr " + out)
